...
return new class implements ModuleContract {
    ...
    {
        $path = __DIR__ . '/Config/' . strtolower('Shop') . '.php';
        if (is_file($path)) {
            $cfg = require $path;
            $current = function_exists('config') ? (array) config(strtolower('Shop'), []) : (array) ($GLOBALS['__ivi_config'][strtolower('Shop')] ?? []);
            $merged = array_replace_recursive($current, (array) $cfg);
            if (function_exists('config_set')) {
                config_set(strtolower('Shop'), $merged);
            } else {
                $GLOBALS['__ivi_config'][strtolower('Shop')] = $merged;
            }
        }
    }
    ...
    {
        \App\Controllers\Controller::addViewNamespace(strtolower('Shop'), __DIR__ . '/views');
